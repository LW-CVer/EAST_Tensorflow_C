...
int East::init(const std::string& ini_path)
{
    ...
    m_box_threshold = reader.GetInteger("threshold", "box_threshold", 0.1);
    m_nms_threshold = reader.GetInteger("threshold", "nms_threshold", 0.2);
    return 0;
}
                                              ...
                                              std::vector<float>& ratios)
{
    ...
    std::vector<std::vector<float>> temp_boxes =
        RestoreRectangle(yx_indexs, coords, angles, scores);
...
std::vector<EastResult> East::detect(cv::Mat& image, double score_threshold)
{
    ...
    cv::Mat resized_img;
    std::vector<float> ratios = ResizeImage(image, resized_img);
    ...
    tf::Status ImageTensorStatus = ReadTensorFromMat(resized_img, image_tensor);
    ...
    //网络特征图相对输入图片缩放4倍
    std::vector<std::vector<float>> results = this->process(
        resized_img.rows / 4, resized_img.cols / 4, score_threshold, ratios);
    std::vector<EastResult> final_results;
    for (auto& result : results) {
        EastResult temp;
        // 0代表了检测的文本
        temp.label = 0;
        temp.score = result[8];
        temp.box_coordinates[0] = result[0];
        temp.box_coordinates[1] = result[1];
        temp.box_coordinates[2] = result[2];
        temp.box_coordinates[3] = result[3];
        temp.box_coordinates[4] = result[4];
        temp.box_coordinates[5] = result[5];
        temp.box_coordinates[6] = result[6];
        temp.box_coordinates[7] = result[7];
